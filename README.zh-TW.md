...
**在 riftbound.chroniclecore.com 上讀一樣東西**：網站自己存在 `localStorage` 的 Supabase 登入狀態，由 `auth-bridge.js` 複製一份到擴充的儲存區。面板能代你掛房間、不必再登入一次，靠的就是這個。它看不到你的密碼——登入是在網站上完成的，這裡只是把結果抄過來；你登出之後這份副本也跟著清掉。

**只跟 `riftbound.chroniclecore.com` 說話**，一律不帶 cookie（`credentials: "omit"`）：

- `GET /api/rooms` — 目前掛在布告欄上的房間。
- `GET /api/card-sets` — 每個系列屬於第幾彈，以及台灣現在開賣到第幾彈。快取一天。這份資料在網站上而不是寫死在擴充裡，所以升彈時你不用更新擴充。
- `POST /api/rooms` — **只在你按下「掛到布告欄」時**，而且只送你自己填的那幾格：房號、稱呼、環境、賽制、備註。用上面那份登入狀態驗證身分。
- `DELETE /api/rooms` — 只在你按下「收掉」時。

**你的牌組與對局內容不會被送到任何地方。** 環境檢查完全在你的瀏覽器裡跑，它讀到的卡號從不外傳；沒有回報，也沒有分析追蹤。除了你親手填的那幾格、在你按下按鈕的那一刻，沒有任何東西離開瀏覽器。所有對外連線都集中在 `background.js` 一個檔案裡，看那一個檔就查得完。
