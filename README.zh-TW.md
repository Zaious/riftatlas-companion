...
**掛房間完全不需要帳號。** 第一次使用時擴充會產生一把 32 bytes 的隨機金鑰，存在擴充的儲存區，掛房間時一起送出。只有帶著同一把金鑰的請求才改得動、收得掉那一間房。伺服器只存它的 SHA-256，所以就算把整張表讀回去，也還原不出任何人的金鑰。換瀏覽器就是另一把，舊的那間房於是改不動了——但它 20 分鐘後本來就自己消失。
...
- `POST /api/rooms` — **只在你按下「掛到布告欄」時**，而且只送你自己填的那幾格：房號、稱呼、環境、賽制、備註，外加上面那把隨機金鑰。
